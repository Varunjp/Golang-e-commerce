...
func Product(c *gin.Context) {
	...
	var bestOffer *OfferView

	if productOffer.ID > 0 && productOffer.DiscountPercentage >= categoryOffer.DiscountPercentage {
		bestOffer = &OfferView{
			ID:                 int(productOffer.ID),
			OfferName:          productOffer.OfferName,
			DiscountPercentage: productOffer.DiscountPercentage,
		}
	} else if categoryOffer.ID > 0 {
		bestOffer = &OfferView{
			ID:                 int(categoryOffer.ID),
			OfferName:          categoryOffer.CategorryName,
	...
	originalPrice := float64(product_variant.Price)

	discountedPrice := originalPrice

	if bestOffer != nil {
		discountedPrice = originalPrice - (originalPrice * bestOffer.DiscountPercentage / 100)
	}
